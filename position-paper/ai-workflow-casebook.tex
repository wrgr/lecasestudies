% The Method Is the Message — a LENS position paper (companion to Capability, Not Content)
...
\hypersetup{
  ...
  pdfsubject={A LENS Position Paper},
...
{\footnotesize\color{lensquote}A position paper of the Capability Matters program \textbullet\ 2026 \textbullet\ Companion to \emph{Capability, Not Content}}
\end{flushleft}
...
\begin{description}[leftmargin=0em,itemsep=3pt,topsep=3pt,font=\color{lensink}\bfseries]
  ...
  \item[Instrument.] Mechanical checks enforced format discipline the way a test suite enforces code: citation parity (every inline marker must have a matching reference), a page-length envelope, and reference-placement rules, run across all 205 cases.
...
\pullquote{AI was the dual entity the curriculum names: a creative partner that accelerated drafting, cross-referencing, and layout---and an epistemic risk that had to be hand-checked against the record.}
...
\begin{description}[leftmargin=1.1em,itemsep=4pt,topsep=3pt,font=\color{lensgood}\bfseries]
  ...
  \item[Tireless mechanical rigor.] Citation parity, anchor completeness, and cross-reference resolution across 205 cases are exactly the checks a human skips when tired. Mechanized, they ran on every case, every build---pre-filling six of the seven verification columns and freeing the human read for the judgment that needs one.
  \item[Structural consistency.] Three anchor systems on every case, a fixed narrative architecture, a grayscale-safe palette---holding a convention across 205 units is where machine assistance shines and human attention frays.
\end{description}
...
The workflow, verification rubric, and reference-validation record summarized here are documented in \emph{Capability Matters: A Casebook} (First Edition, 2026)---see its \textsc{Methodology} (``How this volume was made''), the per-case \textsc{verification log}, and the \textsc{audit} record of citations checked. This paper is a companion to \emph{Capability, Not Content}; together they state the argument and the practice behind it.}
