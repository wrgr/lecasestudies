...
5. **COI disclosure renders prominently under the case title.** When `coi:` is set on a `case(...)` call, the gold-bordered "Disclosure" block renders immediately under the title line, above the Impact rule. **Institutional COI is disclosed even where every editor is personally detached** — detachment changes the *wording*, never the *presence* of the flag. Every `coi:` string is short and clear and **begins with its register word** so the website can label it from the prefix:
   - **`Institutional overlap:`** — an editor shares an institution with ≥1 author (or the deployment site); no editor was personally involved. (e.g., Makary 8, Keystone/Pronovost 19, TREWS 20, Johnson, PBIS 95, JIGSAWS, the Katzman cluster.)
   - **`Authorship:`** — an editor is an author of the underlying study. (CIRCUIT 119/120.)
   - **`Contribution:`** — an editor's group contributed to the work; be explicit about the contribution (the COVID-dashboard standard). (JHU dashboard 179, connectomics/BRAIN 108, 156.)
