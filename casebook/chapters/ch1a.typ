...
#case(
  ...
  competing-readings: (
    [The removed circuit is the decisive absence — set against the reading that the decisive deficit was *feedback*, not shutdown. At Tyler the second dose was delivered because the display said underdose and the fault class permitted a one-key retry; a truthful dosimeter and a suspend-class fault stop the site after the first exposure without any interlock being restored. The corrective action plan deployed hardware and interface changes together, so the record cannot decompose them.],
