...
# Both READMEs quote page counts and the spine width. Those figures were
# hand-maintained and drifted twice — once leaving the pre-press
# instruction naming a spine 0.7 mm narrower than the book needs. Write
# them out here instead, so the next drift is visible rather than silent.
pp() { pdfinfo "build/$1" | awk '/^Pages:/{print $2}'; }
{
  echo "# Product metrics — generated by scripts/build.sh"
  echo
  echo "Do not hand-edit. Re-run \`scripts/build.sh\` to refresh."
  echo
  echo "| Product | Carrier | Pages |"
  echo "|---|---|---|"
  echo "| capability-matters-print.pdf | 8×10 grayscale, 3 mm bleed (Lulu interior) | $pages |"
  echo "| cover-print.pdf | 8×10 Lulu cover wrap | spine **$spine mm** |"
  echo "| capability-matters-local-print.pdf | grayscale interior + colour covers, US Letter | $(pp capability-matters-local-print.pdf) |"
  echo "| capability-matters-digital.pdf | 8×10 main volume, colour on cream | $(pp capability-matters-digital.pdf) |"
  echo "| capability-matters-complete.pdf | 8×10 full standalone, every case | $(pp capability-matters-complete.pdf) |"
  echo "| capability-matters-lens-companion.pdf | 8×10 program docs + crosswalks, white | $(pp capability-matters-lens-companion.pdf) |"
  echo "| capability-matters-validation-audit.pdf | 8×10 audit / tracking / references, white | $(pp capability-matters-validation-audit.pdf) |"
  echo
  echo "Interior padded to a 4-page binding signature; spine = pages × 0.0621 mm."
} > build/product-metrics.md
cp build/product-metrics.md "$REPO_ROOT/products/product-metrics.md"
